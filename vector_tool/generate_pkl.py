...
# Load the JSON data
with open('data/json/merge/output0306.json', 'r') as f:
    data = json.load(f)
...
output_pkl_path = "./data/pkl/output200x300_2423.pkl"
...
def extract_bboxes(image, keypoints):
    ...
    for i in range(0, len(keypoints), 3):
        ...
        if score > 0.0:
            ...
            resized_bbox = cv2.resize(cropped_bbox, (120, 120))
            extracted_bboxes.append(resized_bbox)

    return extracted_bboxes

# Filter bbox_annotations with score >= 0.85
filtered_bboxes = [bbox for bbox in data['bbox_annotations'] if bbox['score'] >= 0.85]
...
disease_entries = [d for d in data['disease'] if d['image_id'] in filtered_image_ids]
...
for disease in disease_entries:
    ...
    disease_class = disease.get('class', '')
    ...
    keypoint_annotations = [kp for kp in data['keypoint_annotations'] if kp['image_id'] == image_id]
    all_bbx_images = []
    ...
    # Ensure exactly 34 bounding boxes per image
    while len(all_bbx_images) < 34:
        all_bbx_images.append(create_black_image(size=(200, 300)))
